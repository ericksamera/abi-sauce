...
class ReferenceAlignmentTraceFigureTheme:
    ...
    event_fill_colors: dict[str, str]


_LIGHT_THEME: Final[ReferenceAlignmentTraceFigureTheme] = (
    ReferenceAlignmentTraceFigureTheme(
        trace_colors={
            "green": "green",
            "blue": "blue",
            "black": "black",
        ...
        event_fill_colors={
            ...
            "mismatch": "rgba(239, 68, 68, 0.14)",
            "insertion": "rgba(59, 130, 246, 0.12)",
            "deletion": "rgba(245, 158, 11, 0.12)",
        },
    )
)

_DARK_THEME: Final[ReferenceAlignmentTraceFigureTheme] = (
    ReferenceAlignmentTraceFigureTheme(
        trace_colors={
            "green": "#22C55E",
            "blue": "#3B82F6",
        ...
        event_fill_colors={
            "match": "rgba(34, 197, 94, 0.12)",
            "mismatch": "rgba(239, 68, 68, 0.18)",
            "insertion": "rgba(59, 130, 246, 0.16)",
            "deletion": "rgba(245, 158, 11, 0.18)",
        },
    )
)
...
def build_reference_alignment_trace_figure(
    ...
    *,
    ...
    """Build an aligned reference-vs-query electropherogram figure."""
    theme = _resolve_figure_theme(theme_type)
    figure = go.Figure()

    reference_bottom = view.total_height + _REFERENCE_BAND_GAP
    ...
    for row in view.rows:
        _add_row_background(figure, row=row, view=view, theme=theme)
        _add_row_channel_segments(figure, row=row, theme=theme)
...
def _add_reference_band(
    ...
    *,
    view: ReferenceAlignmentTraceView,
    theme: ReferenceAlignmentTraceFigureTheme,
    y_bottom: float,
    y_top: float,
    y_text: float,
) -> None:
    _add_background_bars(
        ...
        colors=[_event_fill_color(column.event_type, theme) for column in view.columns],
        ...
        go.Scattergl(
            ...
            customdata=[
                (
                    ...
                    f"<br>reference_base={column.ref_base}"
                    f"<br>query_base={column.query_base}"
                    f"<br>event={column.event_type}"
                    f"<br>ref_pos={column.ref_pos}"
                    f"<br>query_pos={column.query_pos}"
                )
                for column in view.columns
            ],
...
def _add_row_background(
    figure: go.Figure,
    *,
    row: ReferenceAlignmentTraceRow,
    view: ReferenceAlignmentTraceView,
    theme: ReferenceAlignmentTraceFigureTheme,
) -> None:
                ...
                "color": [
                    (
                        _trace_color_for_segment_base(
                            ...
                            fallback_color=theme.query_text_color,
                            theme=theme,
                        )
                        if cell.query_base != "-"
                        else theme.query_text_color
                    )
                    for cell in row.cells
                ]
            },
    ...
    *,
    ...
    figure.add_trace(
        go.Scattergl(
            ...
            hovertemplate=(
                "row=%{customdata[10]}"
                "<br>strand=%{customdata[11]}"
                "<br>column=%{customdata[0]}"
                "<br>reference_base=%{customdata[1]}"
                "<br>query_base=%{customdata[2]}"
                "<br>event=%{customdata[3]}"
                "<br>ref_pos=%{customdata[4]}"
                "<br>query_pos=%{customdata[5]}"
                "<br>quality=%{customdata[6]}"
                "<br>trace_x=%{customdata[7]}"
                "<br>raw_left=%{customdata[8]}"
                "<br>raw_right=%{customdata[9]}"
                "<extra></extra>"
            ),
        )
    )
...
def _row_channel_order(row: ReferenceAlignmentTraceRow) -> tuple[str, ...]:
    for cell in row.cells:
        if cell.channels:
...
def _cell_channel_segment(
    cell: ReferenceAlignmentTraceCell,
    *,
    base: str,
) -> ReferenceAlignmentTraceChannelSegment | None:
...
def _event_fill_color(
    event_type: str,
    theme: ReferenceAlignmentTraceFigureTheme,
) -> str:
    return theme.event_fill_colors.get(event_type, theme.gap_fill_color)
...
def _cell_fill_color(
    ...
    return _event_fill_color(cell.event_type, theme)


def _resolve_figure_theme(theme_type: str) -> ReferenceAlignmentTraceFigureTheme:
    return _DARK_THEME if theme_type == "dark" else _LIGHT_THEME


def _initial_x_range(view: ReferenceAlignmentTraceView) -> tuple[float, float]:
    full_left, full_right = view.x_range
    visible_width = min(
...
__all__ = ["build_reference_alignment_trace_figure"]
