...
def align_page():
    ...
    with left:
        a_id = st.selectbox(
            ...
            format_func=lambda sid: sm.get(sid).name,
        )
    with right:
        b_id = st.selectbox(
            "Sequence B",
            [s.id for s in samples if s.id != a_id],
            format_func=lambda sid: sm.get(sid).name,
        )
    ...
    st.caption(
        f"{sm.get(a_id).name}: {len(seqA)} bp  •  {sm.get(b_id).name}: {len(seqB)} bp"
    )
    ...
    with c3:
        gap_open = st.number_input("gap_open", value=-10.0)
    with c4:
        gap_extend = st.number_input("gap_extend", value=-0.5)

    if st.button("Align"):
        results = pairwise_align(
            seqA,
            seqB,
            mode=mode,
            match=match,
        ...
        mid = "".join(
            "|" if a == b and a != "-" else " " for a, b in zip(r.a_aln, r.b_aln)
        )
        st.code(f"{r.a_aln}\n{mid}\n{r.b_aln}")
